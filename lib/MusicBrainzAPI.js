...
class MusicBrainzAPI {
    constructor(options = {}) {
        ...
        this.user_agent = options.user_agent || `UserJS.MusicBrainzAPI/0.1.0 ( https://musicbrainz.org/user/chaban )`;
    ...
    async lookupUrl(urls, inc = []) {
        const urlArray = Array.isArray(urls) ? [...new Set(urls)] : [urls];
        ...
        if (uncachedUrls.length > 0) {
            try {
                const response = await this._request('url', { resource: uncachedUrls, inc: inc.join('+') });
                ...
                uncachedUrls.forEach(url => this.cache.set(url, null));
            }
        }

        return Array.isArray(urls) ? results : results[urlArray[0]];
    }
